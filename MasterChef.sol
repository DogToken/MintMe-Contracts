...
// Bone with Governance.
contract Bone is BEP20('Bone Token', 'BONE') {
...
// MasterBreeder is the master of Bone. He can make Bone and he is a fair guy.
//
...
contract MasterBreeder is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accBonePerShare;   // Accumulated EBENs per share, times 1e12. See below.
    ...
    function pendingBone(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accBonePerShare = accBonePerShare.add(ebenReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ebenReward = multiplier.mul(emissionPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bone.mint(devaddr, ebenReward.div(10));
        bone.mint(address(this), ebenReward);
        pool.accBonePerShare = pool.accBonePerShare.add(ebenReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterBreeder for BONE allocation.
    ...
    // Safe Bone transfer function, just in case if rounding error causes pool to not have enough EBENs.
    function safeBoneTransfer(address _to, uint256 _amount) internal {
        uint256 bal = bone.balanceOf(address(this));
        if (_amount > bal) {
            bone.transfer(_to, bal);
        } else {

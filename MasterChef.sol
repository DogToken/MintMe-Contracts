...
interface IMigratorChef {
    // Perform LP token migration from legacy PancakeSwap to BoneSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to PancakeSwap LP tokens.
    // BoneSwap must mint EXACTLY the same amount of BoneSwap LP tokens or
...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        bone.mint(address(syrup), boneReward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit BONE by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw BONE by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake BONE tokens to MasterChef
    ...
    // Withdraw BONE tokens from STAKING.
    ...
    function safeBoneTransfer(address _to, uint256 _amount) internal {
        syrup.safeBoneTransfer(_to, _amount);
    }
